...
export default async function InternChannels() {
  const session = await requireRole("intern");
  if (!session) redirect("/login");
  const userId = Number((session.user as any).id);

  const channels = await prisma.channel.findMany({ where: { userId }, include: { user: true, oauthToken: true } });
  const enriched: any[] = [];
  ...
  return (
    <>
      <div className="page-header"><h2>My Channels</h2></div>
      <AddChannelForm />
      <div className="card">
        <div className="card-header"><h3>Tracked Channels ({channels.length})</h3></div>
        {enriched.length > 0 ? (
                ...
                {enriched.map((ch, i) => (
                  <tr key={ch.id}>
                    ...
                    {ch.snap ? (
                      ...
                      <td colSpan={5} className="text-[var(--muted)]">No data</td>
                    )}
          ...
          <div className="text-center py-8 text-[var(--muted)]">No channels yet.</div>
        )}
      </div>
    </>
  );
